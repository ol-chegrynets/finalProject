...
from deck_of_cards import reading, cards_and_suits, selection_card, wins_player

cards_player_1 = []      # карта на роздачі гравця 1
cards_player_2 = []      # карта на роздачі гравця 2
desc_player_1 = []       # колода карт гравця 1
desc_player_2 = []       # колода карт гравця 1
name_player_1 = ""
name_player_2 = ""
...
def distribution_of_cards1(cards):      # розділ колоди на 2 частини без random!
   a = int(len(cards)*0.5)
...
def compare_cards(cards_player_1, cards_player_2):  # порівняти карти на роздачі
   ...
   rank_player_1 = rank_cards_wight.get(cards_player_1[0])+rank_suit.get(cards_player_1[1])
   rank_player_2 = rank_cards_wight.get(cards_player_2[0]) + rank_suit.get(cards_player_2[1])
   if (rank_player_1) > (rank_player_2):
      wins_player(cards_player_1, cards_player_2, desc_player_1)
      print()
      print(f'Victory  {name_player_1}'.center(35))
      print()
   else:
      wins_player(cards_player_2, cards_player_1, desc_player_2)
      print()
      print(f'Victory  {name_player_2}'.center(35))
      print()


def main():
   name_player()
   distribution_of_cards(cards_and_suits(reading("rank.txt"), reading("suit.txt")))
   while True:
      if (len(desc_player_1) == 0):
         print(f' Game End, You lost! Victori {name_player_2}'.center(40))
         break
      if (len(desc_player_2) == 0):
         print(f' Game End, You lost! Victori {name_player_1}'.center(40))
         break
